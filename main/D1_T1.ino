...
void D1_T1_movePath( char steps[] ) {
  ...
  for ( int i=0; i<strlen(steps); i=i+1){
    ...
    while ( state == true) {
      IR_check();
      // junction detect
      if ( (L5_read <= wLine && L3_read <= wLine && L2_read && L1_read <= wLine && R1_read <= wLine) || (L1_read <= wLine && R1_read <= wLine&& R2_read <= wLine && R3_read <= wLine && R5_read <= wLine)) {
        stop_move();
    ...
    // move forward to turning point // 950 for gyro
    move_forward();
    ...
    if (L3_read <= wLine || L1_read <= wLine ||R1_read <= wLine ||R3_read <= wLine ) {
      ...
      move_forward();
      mpu.update();
      float gyroStart = mpu.getAngleZ();
      int timeStart = millis();
      while (( millis() - timeStart)< 500) {
        mpu.update();
        int Gyr0_error = mpu.getAngleZ() - gyroStart;
        if ( Gyr0_error > 0) {
          analogWrite(enR , 200);
          analogWrite(enL , 155);
