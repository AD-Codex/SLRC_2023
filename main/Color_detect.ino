...
void Color_detect() {
  ...
  if( (Sensor_bluePW < Sensor_redPW) && ( Sensor_bluePW < Sensor_greenPW) && Sensor_bluePW < 28){
  ...
  else if(( Sensor_redPW < Sensor_bluePW) && ( Sensor_redPW < Sensor_greenPW) ){
  ...
  else if(( Sensor_greenPW < Sensor_bluePW) && ( Sensor_greenPW < Sensor_redPW)){
...
// Function to read Red Pulse Widths
int Sensor2_getRedPW() {
  // Set sensor to read Red only
  digitalWrite(C2_S2,LOW);
  digitalWrite(C2_S3,LOW);
  // Define integer to represent Pulse Width
  int Sensor_PW;
  // Read the output Pulse Width
  Sensor_PW = pulseIn(C2_Color_sensor_in, LOW);
  // Return the value
  return Sensor_PW;
}

// Function to read Green Pulse Widths
int Sensor2_getGreenPW() {
  // Set sensor to read Green only
  digitalWrite(C2_S2,HIGH);
  digitalWrite(C2_S3,HIGH);
  // Define integer to represent Pulse Width
  int Sensor_PW;
  // Read the output Pulse Width
  Sensor_PW = pulseIn(C2_Color_sensor_in, LOW);
